...
- **`deviceMacro.js`** — A RoomOS (xAPI) macro that runs on a Webex room device. It adds a “Meeting Join” button to the touch UI, lets users enter a passcode, and joins the meeting by initiating a POST request to the Webex Connect launch flow's url.  The launch flow then commands the device to dial in.
...
2. **Room device (macro)**  
   ...
   - Set `connectUrl` in the macro to the URL of the `webex-connect-flow/ic_booking_launch.workflow`.
   - The macro enables the device HTTP client.
   - It adds a “Meeting Join” panel to the UI. When the user enters a meeting code, the macro calls `connectUrl` which corresponds to your imported `ic_booking_launch.workflow)`
...
2. Save the bot's token somewhere secure, we'll need it later.
3. In Control Hub, navigate to the Workspace of the device (not the device itself).
   - Click the 3 dots in the "Devices" panel of the workspace, then select "Edit API Access".
   - Click "Add user or Bot"
   - Begin typing the name of your bot created in step 1.  It should appear in the dropdown. Select it.
   - Give it "Full access" to the workspace.

### Setup (device macro)

1. Open `deviceMacro.js` and set `connectUrl` to your imported `ic_booking_launch.workflow`'s url. Example:
   ```javascript
   const connectUrl = "https://hooks.us.webexconnect.io/events/ABCDEFG123";
   ```
...
Please contact the Webex SD team at [wxsd@external.cisco.com](mailto:wxsd@external.cisco.com?subject=MeetingRoomScheduler) for questions. Or for Cisco internal, reach out to us on Webex App via our bot globalexpert@webex.bot & choose "Engagement Type: API/SDK Proof of Concept Integration Development". 
